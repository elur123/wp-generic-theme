...
$genericstarter_bc_id          = get_queried_object_id();
$genericstarter_bc_full_height = get_post_meta( $genericstarter_bc_id, '_genericstarter_bc_full_height', true ) === '1';
$genericstarter_bc_center      = get_post_meta( $genericstarter_bc_id, '_genericstarter_bc_center', true ) === '1';
$genericstarter_bc_gradient    = get_post_meta( $genericstarter_bc_id, '_genericstarter_bc_gradient', true ) === '1';

// Lock to the viewport when full-height is on; otherwise grow with content.
$genericstarter_bc_height_class = $genericstarter_bc_full_height
...
$genericstarter_bc_layout_class = $genericstarter_bc_center
	? 'flex flex-col justify-center text-center'
	: 'flex flex-col';

// Build the secondary radial-gradient background, if enabled.
$genericstarter_bc_style = '';
if ( $genericstarter_bc_gradient ) {
	$genericstarter_bc_color = get_post_meta( $genericstarter_bc_id, '_genericstarter_bc_gradient_color', true );
	$genericstarter_bc_color = $genericstarter_bc_color !== '' ? $genericstarter_bc_color : '#f25f5a';

	$genericstarter_bc_y = get_post_meta( $genericstarter_bc_id, '_genericstarter_bc_gradient_y', true );
	$genericstarter_bc_y = $genericstarter_bc_y !== '' ? (int) $genericstarter_bc_y : 50;

	$genericstarter_bc_style = sprintf(
		'background-image:radial-gradient(60%% 60%% at 50%% %d%%, %s, transparent 70%%);',
		$genericstarter_bc_y,
		$genericstarter_bc_color
	);
}
?>
<!doctype html>
...
<?php if ( $genericstarter_bc_style ) : ?>
	...
	<div class="msp-bc-gradient fixed inset-0 z-0 pointer-events-none"
		style="<?php echo esc_attr( $genericstarter_bc_style ); ?>"
		aria-hidden="true"></div>
<?php endif; ?>

<main id="main"
	class="site-main relative z-10 <?php echo esc_attr( $genericstarter_bc_height_class ); ?> <?php echo esc_attr( $genericstarter_bc_layout_class ); ?>"
	role="main">
