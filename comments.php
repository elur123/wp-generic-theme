...
<div id="comments" class="comments-area mt-12 pt-10 border-t border-neutral-200 dark:border-neutral-700">
	...
	<h2 class="comments-title text-xl font-bold mb-8">
		...
		if ( '1' === $comment_count ) {
			printf(
				...
				esc_html__( 'One thought on &ldquo;%s&rdquo;', 'medspastarter' ),
				'<span>' . wp_kses_post( get_the_title() ) . '</span>'
			);
		} else {
			printf(
				/* translators: 1: comment count, 2: post title */
				esc_html( _nx( '%1$s thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;', $comment_count, 'comments title', 'medspastarter' ) ),
	...
	<ol class="comment-list space-y-8 mb-10">
		...
		wp_list_comments( [
			...
			'short_ping'  => true,
			'avatar_size' => 48,
			'callback'    => 'medspastarter_comment_template',
		] );
		?>
	</ol>
	...
	the_comments_navigation( [
		'prev_text' => esc_html__( '&larr; Older comments', 'medspastarter' ),
		'next_text' => esc_html__( 'Newer comments &rarr;', 'medspastarter' ),
	] );
	?>
	...
	<p class="no-comments text-sm text-neutral-700/60 dark:text-neutral-500">
		<?php esc_html_e( 'Comments are closed.', 'medspastarter' ); ?>
	</p>
	<?php endif; ?>

	<?php
	comment_form( [
		'title_reply'          => esc_html__( 'Leave a comment', 'medspastarter' ),
		'title_reply_to'       => esc_html__( 'Reply to %s', 'medspastarter' ),
		...
		'comment_notes_before' => '<p class="comment-notes text-sm text-neutral-700/60 dark:text-neutral-500 mb-4">'
			. esc_html__( 'Your email address will not be published.', 'medspastarter' )
			. '</p>',
		'fields'               => [
			'author' => '<div class="grid sm:grid-cols-2 gap-4 mb-4"><p class="comment-form-author">
				<label for="author">' . esc_html__( 'Name', 'medspastarter' ) . ' <span aria-hidden="true">*</span></label>
			...
			'email'  => '<p class="comment-form-email">
				<label for="email">' . esc_html__( 'Email', 'medspastarter' ) . ' <span aria-hidden="true">*</span></label>
		...
		'comment_field'        => '<p class="comment-form-comment mb-4">
			<label for="comment">' . esc_html__( 'Comment', 'medspastarter' ) . ' <span aria-hidden="true">*</span></label>
